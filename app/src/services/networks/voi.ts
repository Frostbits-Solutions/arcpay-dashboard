...
async function getAddressAssets(algodClient: algosdk.Algodv2, address: string, network: string, page: number = 0, size: number = 25): Promise<AssetMetadata[]> {
  let url
...
async function getCreatedAppId(algodClient: algosdk.Algodv2, txId: string, network: string): Promise<number> {
  ...
  const getTxExponentialBackOff = (tryNumber: number = 0): Promise<number> => {
    ...
    return new Promise<number>((resolve, reject) => {
      setTimeout(() => {
        axios
          .get<{ transaction: { 'created-application-index'?: number } }>(url as string)
          .then((response) => {
