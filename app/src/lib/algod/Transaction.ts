import type { AppCallObject, AppCreateObject, AppDeleteObject, AppObject, PaymentObject, TransactionObject, TransfertObject } from './types'
...
import { base64ToArrayBuffer, encodeAppArgs, getRekeyAddress, makeEmptyTransactionSigner } from './utils'
...
export class Transaction {
  ...
  public createApp(args: Uint8Array[], approvalProgram: string, clearProgram: string, numGlobalInts?: number, numGlobalByteSlices?: number, numLocalInts?: number, numLocalByteSlices?: number) {
    this._queue.push({
      method: '_createApp',
      args: [args, approvalProgram, clearProgram, numGlobalInts, numGlobalByteSlices, numLocalInts, numLocalByteSlices],
    })
    return this
  }
  ...
  public async send(transactionSigner: TransactionSigner): Promise<TransactionConfirmation> {
    ...
    return new Promise<TransactionConfirmation>((resolve, reject) => {
      atc
        .execute(this._algod, 4)
        .then(({ confirmedRound, txIDs, methodResults }) => {
  ...
  private async _fund(amount?: number) {
    ...
    const fundingAmount = amount || 500_700_000
    ...
    if (results?.txnGroups[0]?.unnamedResourcesAccessed?.boxes) {
      ...
      for (const obj of this._objs) {
        ...
        appObj.boxes = results.txnGroups[0].unnamedResourcesAccessed.boxes.map((box) => {
          if (box.app !== 0 && (!('appIndex' in appObj) || box.app !== appObj?.appIndex) && !foreignApps.includes(box.app as number)) {
