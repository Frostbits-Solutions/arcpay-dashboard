...
const onNetworkParamFormSubmit = form.handleSubmit(async (values) => {
  ...
  const apiCall = existingNetworkParameter ? updateAccountNetworksParameters : createAccountNetworksParameters
  ...
  if (error) {
    errorHandler(error, `Error ${existingNetworkParameter ? 'updating' : 'creating'} network parameter for ${networkId}.`)
    return
  }
