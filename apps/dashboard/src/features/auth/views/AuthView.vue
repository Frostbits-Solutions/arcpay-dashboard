  ...
  <div class="flex h-screen items-center justify-center bg-muted/30">
    <div class="w-full max-w-sm rounded-xl border border-border bg-popover p-4 shadow-xl sm:p-6 md:p-8">
      <div class="mb-4 flex items-center justify-center">
        <img :src="logo" alt="Logo" class="mr-2 h-14" />
        <h1 class="text-5xl text-foreground">arcpay</h1>
      </div>
      <form v-if="!success" class="mt-8 border-t border-border pt-10" @submit.prevent="handleLogin">
        <div>
        ...
        <Button variant="gradient" type="submit" size="lg" class="mb-2 mt-12 w-full">
        ...
        <div v-if="register" class="text-center text-xs font-medium text-muted-foreground">
          Already registered?
          <a @click.prevent="register = false" href="#" class="text-blue-700 hover:underline dark:text-blue-500">Sign-in</a>
        </div>
        <div v-else class="text-center text-xs font-medium text-muted-foreground">
          Not registered?
          <a @click.prevent="register = true" href="#" class="text-blue-700 hover:underline dark:text-blue-500">Create account</a>
        </div>
      </form>
      <div v-else class="space-y-6 border-t pt-8 text-center text-gray-700 dark:border-gray-700 dark:text-gray-200">Check your email for the login link!</div>
    </div>
  </div>
</template>
